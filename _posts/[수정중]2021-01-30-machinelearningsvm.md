...
## <span style="color:darkblue">1. Kernel method</span>
...
### <span style="color:darkblue">1.1. Kernel Trick</span>

![함정카드](/assets/img/MATLAB/10_2.png)

 **아니, 이런 학술적인 용어에 트릭이요?**

 먼저, `운영체제(OS:Operating System)` 에서의 하드웨어와 중개자역할인 `Kernel` 과 개념적으로 유사하나 그에 대한 설명이 아닙니다. `Kernel Trick` 은 **실제 차원에 대해서 공간을 변환하지 않고 우회하여 변환하는 효과를 주는 것입니다.**  말그대로 `Trick` 이라서 `선형 분리` 가 불가능한 공간을 `선형 분리` 가 가능한 차원 혹은 공간 변환의 효과를 내는 것이죠.
